...
import com.rk.utils.InvoiceGenerator;
	...
	public PaymentResponse createPaymentLink(Long paymentId) throws StripeException {
		...
		SessionCreateParams params = SessionCreateParams.builder().setMode(SessionCreateParams.Mode.PAYMENT)
				...
				.setPaymentIntentData(SessionCreateParams.PaymentIntentData.builder()
						.putMetadata("paymentId", paymentId.toString()).build())
				.addLineItem(SessionCreateParams.LineItem.builder().setPriceData(SessionCreateParams.LineItem.PriceData
						.builder().setCurrency("inr").setUnitAmount((long) (payment.getAmount() * 100))
						.setProductData(SessionCreateParams.LineItem.PriceData.ProductData.builder()
								.setName("Hostel Payment - " + payment.getBooking().getStudent().getFullName()).build())
...
//    	
		...
		return new RazorpayOrderResponse(order.get("id"), payment.getAmount(), "INR", payment.getId(), studentName,
	...
	public boolean updateRazorpayWebhook(String payload, String sigHeader) throws Exception {
		try {
			...
			if ("payment.captured".equals(event)) {
				JSONObject paymentEntity = json.getJSONObject("payload").getJSONObject("payment")
				...
				Booking booking = bookingRepository.findById(payment.getBooking().getId())
						.orElseThrow(() -> new AppException("Invalid booking"));
				...
				notificationService.createNotification(student, "Payment received by Owner Successfully!");

				// generate and send invoice
				String razorpayPaymentId = paymentEntity.getString("id");
				String filePath = InvoiceGenerator.generateInvoice(student.getFullName(), student.getEmail(),
						payment.getAmount(), razorpayPaymentId, payment);
				emailService.sendInvoice(student.getEmail(), filePath);
				...
				if (file.exists()) {
					...
					if (deleted) {
						System.out.println("✅ Invoice file deleted successfully: " + filePath);
					} else {
						System.out.println("⚠️ Failed to delete invoice file: " + filePath);
